...
public class DataProcessReader extends Reader<DataProcess> {
    ...
    public List<DataProcess> read(QTRestClient client) {
        Request request = new Request(HttpMethod.GET, "/search");

        Response response = client.request(request);

        if (response == null) {
            throw new QTApiConnectionException("DataProcess read failed: Unable to connect to server");
        ...
        return DataProcess.listFromJson(response.getStream(), client.getObjectMapper());
    }
}
